...
func TestNew(t *testing.T) {
	tests := []struct {
		...
		{nil, []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{[]byte{}, []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{[]byte(""), []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{[]byte("hello"), []byte{0x00, 0x00, 0x00, 0x68, 0x65, 0x6c, 0x6c, 0x6f}},
		{[]byte("hello123"), []byte{0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x31, 0x32, 0x33}},
		{[]byte("hello123abc"), []byte{0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x31, 0x32, 0x33}},
	}
	for i, test := range tests {
		g := New(test.value)
		if bytes.Compare(g.id, test.expected) != 0 {
			t.Errorf("%d: got %v, want %v", i, g.id, test.expected)
		}
	}
}
